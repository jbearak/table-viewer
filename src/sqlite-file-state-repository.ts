...
function decode_state_json(value: unknown): { json: string; state: StoredPerFileState } {
    ...
    try {
        return { json, state: decode_stored_per_file_state(parsed) };
    } catch {
        return malformed();
    }
}
...
function validate_entry(value: PersistedKeyedStateEntry): void {
    ...
    const decoded = decode_state_json(value.stateJson);
    if (state_has_pending_edits(decoded.state) !== value.hasPendingEdits) malformed();
}
...
export class SqliteFileStateRepository implements KeyedStateWriteTransaction {
    ...
    read_entry(path: string): PersistedCompleteKeyedStateEntry | undefined {
        const row = this.#tx.prepare(`SELECT ${ENTRY_METADATA_COLUMNS}, e.state_json
            FROM entries e WHERE e.path = ?`).get(path);
    ...
    delete_authority_stages_before(boundary: number): readonly string[] {
        ...
        const paths = tx.prepare(`SELECT DISTINCT entry_path
            FROM authority_stages INDEXED BY authority_stages_by_age
            WHERE created_at_ms < ? ORDER BY entry_path`).all(safeBoundary)
