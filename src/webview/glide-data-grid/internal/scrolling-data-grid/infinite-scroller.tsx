...
// De-Linaria'd: rules live in ../../styles.css under .gdg-scroll-region; the
// Safari overflow choice becomes a modifier class.
const ScrollRegionStyle: React.FC<React.PropsWithChildren<{ isSafari: boolean }>> = p => (
    <div className={"gdg-scroll-region" + (p.isSafari ? " gdg-scroll-region-safari" : "")}>{p.children}</div>
);
...
export const InfiniteScroller: React.FC<React.PropsWithChildren<Props>> = p => {
    const {
        children,
        clientHeight,
        scrollHeight,
