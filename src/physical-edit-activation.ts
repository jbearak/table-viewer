...
function ephemeral_store(context: vscode.ExtensionContext): AuthorityFileStateStore {
    let envelope = context.globalState.get<unknown>(MEMENTO_STATE_KEY, {});
    const medium: FileStatePersistenceMedium = {
        runtime_key: {},
        read: () => envelope,
        write: async (next) => { envelope = next; },
    };
    return create_authority_store(medium, () => Math.max(
        1,
...
export async function create_physical_edit_activation_boundary(
    ...
    const store = switching_store(
        create_file_state_store(context, () => Math.max(
            1,
            vscode.workspace.getConfiguration('tableViewer').get<number>('maxStoredFiles', 10000),
        )),
        ephemeral_store(context),
        () => phase === 'viewOnly',
        track,
    );
    const drain = async () => {
...
export async function run_physical_edit_protocol_setup(
    ...
    if (accepted !== attestation) return false;
