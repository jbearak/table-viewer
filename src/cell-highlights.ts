...
export function sanitize_sheet_cell_highlights(
    ...
    return { schema: value.schema, cells };
}
...
export function sanitize_cell_highlight_state(
    ...
    for (const stored_sheet of value.sheets) {
        const sanitized = sanitize_sheet_cell_highlights(stored_sheet);
        sheets.push(sanitized);
        highlighted_cell_count += sanitized
            ? Object.keys(sanitized.cells).length
            : 0;
    }
    if (highlighted_cell_count === 0) return undefined;
    return { sourceDigest: value.sourceDigest, sheets };
}

/** Derive the current webview payload without mutating or replacing durable state. */
export function project_renderable_cell_highlight_state(
    value: unknown,
    meta: WorkbookMeta,
    current_digest?: string | null,
): CellHighlightState | undefined {
    const durable = sanitize_cell_highlight_state(value);
    if (!durable) return undefined;

    const sheets = meta.sheets.map((sheet, index) => {
        const stored_sheet = durable.sheets[index];
        if (!stored_sheet) return undefined;
        const cells = new Map<string, CellHighlightColor>();
        for (const [key, color] of Object.entries(stored_sheet.cells)) {
            const coordinates = parse_cell_highlight_key(key)!;
            if (
        ...
        return Object.keys(ordered_cells).length > 0
            ? { schema: transform_schema_for_sheet(sheet), cells: ordered_cells }
            : undefined;
    });
    if (sheets.every((entry) => entry === undefined)) return undefined;
    return {
        sourceDigest: typeof current_digest === 'string'
            ? current_digest
            : durable.sourceDigest,
        sheets,
    };
}
...
export function apply_cell_highlight_patch(
    ...
    const sanitized_current = sanitize_cell_highlight_state(current);
    ...
    if (is_record(patch.cells)) {
        for (const [key, candidate_color] of Object.entries(patch.cells)) {
            ...
            const color = sanitize_cell_highlight_color(candidate_color);
            if (color) {
                if (!cells.has(canonical_key)) additions += 1;
                cells.set(canonical_key, color);
            }
    ...
    if (
        additions > 0
        && count_cell_highlights(result) > MAX_HIGHLIGHTED_CELLS_PER_FILE
    ) {
...
export function reconcile_physical_cell_highlights(
    ...
    return rebase_cell_highlight_digest(current, physical_digest);
}
...
export function rebase_cell_highlight_digest(
    ...
    const sanitized = sanitize_cell_highlight_state(current);
    return sanitized
...
export function migrate_cell_highlight_schema(
    ...
    const existing = sanitize_sheet_cell_highlights(current.sheets[sheet_index]);
    if (!existing) return current;

    const next_schema = transform_schema_for_sheet(new_sheet);
    const sheets = current.sheets.slice();
    sheets[sheet_index] = { schema: next_schema, cells: existing.cells };
    return { sourceDigest: current.sourceDigest, sheets };
}
