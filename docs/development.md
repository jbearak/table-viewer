...
`release-build.yml` first runs the Windows SQLite public-open and packaged/runtime recovery gates against the immutable tag commit. Only after they pass does `build` package and upload the `.vsix` on Linux, and `desktop-windows` package the x64 and arm64 setup and portable executables. The `desktop` job packages the standalone macOS app (arm64 dmg + zip). The desktop jobs also validate and retain every update channel: `latest-mac.yml`, the installed Windows `latest.yml` / `latest-arm64.yml`, the portable Windows `latest-portable.yml` / `latest-portable-arm64.yml`, and the Windows setup blockmaps. Each Windows manifest contains one architecture and one distribution form. `release-publish.yml` revalidates that each manifest names the matching version/architecture payload, publishes the extension to both marketplaces, attaches all updater metadata with the binaries in the same GitHub Release, and seeds or updates the Homebrew tap. See the [Homebrew tap guide](homebrew-tap.md) for that flow, its one-time setup, and how code signing switches itself on.
