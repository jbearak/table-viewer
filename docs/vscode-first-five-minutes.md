...
- In VS Code, choose **File → Open File…**, select an `.xlsx`, `.xls`, `.csv`, `.tsv`, `.dta`, or `.arrow` file, and click **Open**.
...
>
> `.xlsx`, `.csv`, and `.tsv` files are editable. Legacy `.xls` workbooks, `.dta` datasets, and `.arrow` files are read-only.
