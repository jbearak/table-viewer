...
If VS Code is not already installed, [download Visual Studio Code](https://code.visualstudio.com/) and follow its normal installation steps. Table Viewer requires VS Code 1.127 or later. You do not need to create a project, open a programming folder, or configure anything else.
...
- Modern `.xlsx` workbooks and `.parquet`, `.csv`, and `.tsv` files have an optional edit mode; legacy `.xls` workbooks and `.dta` datasets are read-only. Edit mode changes only the contents of cells, and only once you save. The extension never writes your sorts, filters, widths, hidden columns, or highlights into any format.
