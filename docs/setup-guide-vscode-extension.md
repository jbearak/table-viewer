...
- Modern `.xlsx` workbooks, `.csv` files, and `.tsv` files have an optional edit mode; legacy `.xls` workbooks and `.dta` datasets are read-only. Edit mode changes only the contents of cells, and only once you save. The extension never writes your sorts, filters, widths, hidden columns, or highlights into any format.
