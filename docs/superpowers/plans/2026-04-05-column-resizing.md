...
Create `src/webview/measure-column.ts`. Process `merges` into a `hidden` set only; `multi_col_merges` was removed because multi-column spans are skipped naturally when the cell lookup only measures cells where `col_span === 1`:
...
Update the `Toolbar` function to destructure and render the new button. The toolbar should still render when only the always-present auto-fit button is available, so remove the old null-return guard:
