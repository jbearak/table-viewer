...
The release workflow runs the same command on Windows after the SQLite/runtime release gates pass, uploads all four executables with checksums, and requires that artifact before publishing the GitHub Release. Windows builds are unsigned, so SmartScreen shows its first-run warning.
...
- Desktop preparation runs Electron's proxy-aware, idempotent installer before bundling, retrying transient download failures up to three times. This repairs an install whose binary download was skipped or interrupted; a complete extraction is left untouched.
