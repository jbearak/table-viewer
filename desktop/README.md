...
The release workflow runs the same command on Windows after the SQLite/runtime release gates pass, uploads all four executables with checksums plus the installed-build update manifests and blockmaps, and requires that artifact before publishing the GitHub Release. Windows builds are unsigned, so SmartScreen shows its first-run warning.

Four exes land in `dist/desktop-packages/`: `table-viewer-<version>-<arch>-setup.exe` and `table-viewer-<version>-<arch>-portable.exe` for each of `x64` and `arm64`. Installed setup builds check GitHub Releases automatically and expose **Help → Check for Updates…**; portable builds do neither and must be replaced manually. Packaging runs each architecture separately so `latest.yml` selects x64 while `latest-arm64.yml` selects arm64. The per-target `artifactName` overrides in `electron-builder.yml` exist because both targets emit `.exe` and would otherwise collide on the shared name, and `nsis.buildUniversalInstaller: false` is what splits the installer per architecture instead of shipping one that carries both payloads.
