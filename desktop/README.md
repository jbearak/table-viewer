...
The release workflow runs the same command on Windows after the SQLite/runtime release gates pass, uploads all four executables with checksums plus the installed and portable update manifests and setup blockmaps, and requires that artifact before publishing the GitHub Release. Windows builds are unsigned, so SmartScreen shows its first-run warning.

Four exes land in `dist/desktop-packages/`: `table-viewer-<version>-<arch>-setup.exe` and `table-viewer-<version>-<arch>-portable.exe` for each of `x64` and `arm64`. Both forms check GitHub Releases automatically and expose **Help → Check for Updates…**. Installed builds use `latest.yml` / `latest-arm64.yml` and electron-updater's NSIS flow. Portable builds use the isolated `latest-portable.yml` / `latest-portable-arm64.yml` channels, download the complete replacement executable, and hand it to a small native helper after the normal viewer/SQLite quit fence. The portable file's directory must be writable; the updater never elevates automatically. It keeps the old executable as a backup until the replacement reaches a healthy startup acknowledgement, rolling back if replacement or relaunch fails. Packaging runs each architecture separately so every manifest selects exactly one architecture and distribution form. The per-target `artifactName` overrides in `electron-builder.yml` exist because both targets emit `.exe` and would otherwise collide on the shared name, and `nsis.buildUniversalInstaller: false` is what splits the installer per architecture instead of shipping one that carries both payloads.

File associations for the installer are registered by `desktop/installer.nsh` (wired in via `nsis.include`), not by electron-builder's `fileAssociations`. The generated ones write the *default* value of `HKCU\Software\Classes\.csv` — which is how Windows records the default program — so installing would have taken `.xlsx` from Excel. The hand-written version writes a vendor-prefixed ProgID plus an `OpenWithProgids` entry instead, so the app shows up in "Open with…" and the user's default is untouched. This is also why `fileAssociations` is scoped under `mac:` in the config: electron-builder concatenates the root and per-platform lists, so a top-level entry cannot be kept away from Windows.

electron-builder does not apply `nsis.include` to the portable target. On portable startup, `desktop/main/windows-file-associations.ts` therefore registers separate `TableViewerPortable.*` ProgIDs under `HKCU`, pointing at the stable outer portable exe rather than its temporary extracted copy. It uses the same extension and description set as the installer, updates moved-executable paths on the next launch, and likewise leaves current defaults untouched. These registrations are required for `app.addRecentDocument` to populate the Windows Jump List. Because a portable exe has no uninstaller, deleting it can leave those per-user entries behind.
...
- File associations declare the app as *a* handler for `csv`/`tsv`/`xlsx`/`xls`/`parquet`/`dta` (`rank: Alternate`) so it appears in "Open with…" without claiming default-handler status.
...
The Color scheme preference (`theme`) is `system` by default, which follows the OS light/dark setting; `light` and `dark` pin it. It is applied by handing the value to Electron's `nativeTheme.themeSource`, so Color scheme only picks the *mode*. Two further preferences pick which theme paints each mode: `lightThemeId` and `darkThemeId`. Keeping a slot per mode means switching Color scheme back and forth never loses the theme chosen for the other mode.

`resolve_theme_id` in `desktop/main/theme-definitions.ts` is the single place the active theme is computed (mode from `nativeTheme.shouldUseDarkColors`, theme from the matching slot), so no call site can get the mode right while ignoring the theme choice. `theme-definitions.ts` is also the registry of the shipped themes — the two hand-tuned VS Code look-alikes (Light, Dark), the original Macher Light/Dark pair, and ports of Solarized, Catppuccin, Gruvbox, SynthWave '84, Cyberpunk, VS Code Red, and VS Code's Dark and Light High Contrast themes. Their semantic roles are expanded into the full `--vscode-*` set by `desktop/main/theme-palette.ts`. Ported palettes are attributed in [NOTICE.md](../NOTICE.md).

The Preferences **Color theme** select is a view of the live theme payload rather than of the settings file, so it lists the themes for whichever mode is resolved *right now* and retargets itself when the OS flips under Color scheme = System.
...
The Edit menu is hand-built rather than `role: 'editMenu'`. The stock menu's Delete and Paste and Match Style items have nothing to act on — the grid is a canvas with no DOM selection — so they are omitted, and its Undo and Redo roles reach only the focused page's *text* undo, never the workbook history. Cut and Paste keep their native roles, because the one place they mean anything is the CSV cell editor's text field, which is exactly what those roles operate on.

Undo and Redo are custom items that name the gesture they would apply — "Undo Paste" — and grey out when the stack is empty. The history lives in the renderer, which a native menu cannot read, so the webview posts a `historyMenuStateChanged` projection of it (`src/webview/history-menu-projection.ts`); `viewer-windows.ts` retains the latest one per window and asks main.ts to rebuild the application menu when it is the focused window's. The rebuild is not an economy measure avoided — a MenuItem's label is fixed at construction, so a changing label *means* a new menu — and `desktop/main/history-menu-model.ts` answers what each item should read.

Both items stay enabled for a window with no history model: the welcome window, Preferences, the state inspector, and a viewer whose focus is in a cell editor's text field. That is not a fallback but the rule — those windows still have the page's own text undo, a different stack whose contents nothing in the main process can see, and `Cmd/Ctrl+Z` inside an open cell editor must remain that text undo. A replay already in flight changes neither item; the renderer refuses a second replay silently, and greying out for the few hundred milliseconds one takes would flicker under a held-down chord.

Redo carries one accelerator per platform (`Cmd+Shift+Z` on macOS, `Ctrl+Y` elsewhere) because a menu item can carry only one. The renderer accepts either chord when it sees a keystroke directly.
