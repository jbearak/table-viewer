...
Review and compare spreadsheets, tables, and datasets, with editing for common formats.

![Table Viewer comparing two recipe tables, with added, deleted, moved, and changed rows](docs/images/app-recipe-diff.png)

Table Viewer supports editing modern Excel (`.xlsx`), `.csv`, and `.tsv` files, plus read-only viewing and diffing of legacy Excel (`.xls`) workbooks, Stata (`.dta`) datasets, and Arrow (`.arrow`) files. It is built for reviewing, exploring, comparing, and annotating tables. Available as a standalone desktop app and a VS Code extension.

What you do while reviewing a table — sorting, filtering, hiding columns, resizing, highlighting cells — is stored alongside the file rather than inside it, so the file on disk is untouched and your sorts, filters, and highlights are still there the next time you open it, even if the file was regenerated in the meantime. Table Viewer also reloads on its own when the file changes on disk.

<img src="docs/images/app-column-header-menu-hero.png" alt="The Roast column's context menu in Table Viewer, with sorting, filtering, copying, and hiding actions" width="751">
...
You can also open your editor's **Extensions** view and search for the exact extension ID `jbearak.table-viewer`. After installation, open an `.xlsx`, `.xls`, `.csv`, `.tsv`, `.dta`, or `.arrow` file; Table Viewer registers itself as an editor for those file types.
...
- **Diffing a spreadsheet in Git shows you a wall of text.** Click a changed `.xlsx`, `.xls`, `.csv`, `.tsv`, `.dta`, or `.arrow` file in VS Code's Source Control or Timeline view and Table Viewer opens the comparison as a table: added and deleted rows banded, changed cells showing before and after in place. The desktop app also compares any two files.
...
**Viewing**
- Opens `.xlsx`, `.xls`, `.csv`, `.tsv`, `.dta`, and `.arrow` files
...
**Table diffs**
- Compare two Excel, Stata, Arrow, CSV, or TSV files in a read-only table view — neither file is modified
...
In VS Code, click a changed Excel, Stata, Arrow, CSV, or TSV file in the Source Control or Timeline view and Table Viewer opens the comparison.
...
**Arrow files** (`.arrow`) open as single read-only tables, including ordinary Arrow IPC files and dtatools-profile datasets. They support the same viewing and comparison workflows. See [Arrow file support](docs/arrow-files.md) for types, exact values, compression, and memory behavior.

## Default editor behavior

Table Viewer registers as the default editor for Excel (`.xlsx`, `.xls`), Stata (`.dta`), Arrow (`.arrow`), CSV, and TSV files. `.xlsx`, CSV, and TSV files are editable; legacy `.xls` workbooks, `.dta` datasets, and `.arrow` files are read-only. If another viewer is registered for one of these formats, VS Code may ask which editor you'd like to use.
